...
- Active root path is now a `#1060`-derived 11-layer `#549`-family scaffold with coprime multi-shard loading, reserved-time full-Hessian GPTQ6, and no TTT carryover.
...
# GREEN / YELLOW / RED
- `GREEN`: `#1060`-derived root with reserved-time full-Hessian GPTQ6 and prune-funded `BigramHash(3072,112)`.
- `YELLOW`: completed `#549` / `#589` truth runs are reference-only; exact `#1060` donor parity is local-sanity-only, not an H100 target.
...
- The highest expected-value next lane is a `#1060`-derived 11-layer scaffold with one byte-funded delta, not another replay of the accepted donor family.
...
- The single promoted delta is `BigramHash(3072,112)` funded by selective `±1` export pruning, not another eval mechanism.
