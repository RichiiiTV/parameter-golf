...
class PreNormCastedLinear(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, *, bias: bool = False, use_preproj_rmsnorm: bool = False):
        super().__init__()
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.c_q = PreNormCastedLinear(dim, dim, bias=False, use_preproj_rmsnorm=use_preproj_rmsnorm)
        self.c_k = PreNormCastedLinear(dim, kv_dim, bias=False, use_preproj_rmsnorm=use_preproj_rmsnorm)
        self.c_v = PreNormCastedLinear(dim, kv_dim, bias=False, use_preproj_rmsnorm=use_preproj_rmsnorm)
        self.proj = PreNormCastedLinear(dim, dim, bias=False, use_preproj_rmsnorm=use_preproj_rmsnorm)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: float, use_preproj_rmsnorm: bool):
        super().__init__()
        hidden = int(mlp_mult * dim)
        self.fc = PreNormCastedLinear(dim, hidden, bias=False, use_preproj_rmsnorm=use_preproj_rmsnorm)
        self.proj = PreNormCastedLinear(hidden, dim, bias=False, use_preproj_rmsnorm=use_preproj_rmsnorm)
        self.proj.linear._zero_init = True
...
class BigramHashEmbedding(nn.Module):
    ...
    def __init__(self, bigram_vocab_size: int, bigram_dim: int, model_dim: int, use_preproj_rmsnorm: bool):
        super().__init__()
        self.bigram_vocab_size = bigram_vocab_size
        self.embed = nn.Embedding(bigram_vocab_size, bigram_dim)
        nn.init.zeros_(self.embed.weight)
        self.proj = (
            PreNormCastedLinear(bigram_dim, model_dim, bias=False, use_preproj_rmsnorm=use_preproj_rmsnorm)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(
            dim, num_heads, num_kv_heads, rope_base, qk_gain_init, use_preproj_rmsnorm
        )
        self.mlp = MLP(dim, mlp_mult, use_preproj_rmsnorm)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bigram = (
            BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim, use_preproj_rmsnorm)
        ...
        self.blocks = nn.ModuleList(
            [
                Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, use_preproj_rmsnorm)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        use_preproj_rmsnorm=args.use_preproj_rmsnorm,
    ).to(device).bfloat16()
    ...
    log0(f"use_preproj_rmsnorm:{int(args.use_preproj_rmsnorm)}")
    log0(
